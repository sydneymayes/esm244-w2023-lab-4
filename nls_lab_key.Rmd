...
These fishery models are easily fit by nls, but sometimes we need to add more controls. I want to emphasize, this is unnecessary for these data and models. I'm only including it to show how you would do it and what happens.

```{r}
# default settings nls.control(maxiter = 50, tol = 1e-05, minFactor = 1/1024,
            #printEval = FALSE, warnOnly = FALSE)

control_nls=nls(surplus~fox(m,carry,TBbest),
                  data=one_stock,
                  start=list(m=guess_vec[1],carry=guess_vec[2]),
                  control = nls.control(tol = 2.1e-9,minFactor=1e-10,warnOnly = TRUE))
...
Huzzah! We improved our carrying capacity coefficient by .01! But was it really worth it? Not really. In more complex models you may need to increase the max iterations to make sure the algorithm has time to converge. That will increase the time it takes to run however, so find a balance between accuracy and time. If you do need to be even more accurate you can up the tolerance, but remember the default is already saying we need to see a change of only 0.0001% difference. That's very precise. 
